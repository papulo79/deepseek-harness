...
describe('ui-theme apply', () => {
  ...
  it('loads Host settings at boot and refreshes its namespace', async () => {
    const b = await bench()
  ...
  it('loads and writes the Host document from a non-loopback browser too', async () => {
    const b = await bench(false)
    // Any loaded page is authenticated to this Host, so page locality changes
    // nothing about the settings transport.
    b.setHostSection({ preference: 'dark', fontSize: 17 })
    b.events.emit('settings/document-updated', [THEME_SETTINGS_NAMESPACE, 0])
    declareItems(b.slots)
    await b.ctx.plugin({ inject: [...inject], apply }).await()
