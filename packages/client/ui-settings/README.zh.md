...
- **每个 DSH home 只有一份 settings 文档**：一个 Host 的每个已认证浏览器都编辑同一份 profile 文档（`$DSH_HOME/cordis.patch.yml`），因此配对后的 LAN 浏览器与回环页面共享同一份文档；只有 Host 文档动作仍仅限回环（ui-settings-general）。
