...
- **One settings document per DSH home** — every authenticated browser of a Host edits the same profile document (`$DSH_HOME/cordis.patch.yml`), so a paired LAN browser and the loopback page share one document; only the Host document action stays loopback-only (ui-settings-general).
