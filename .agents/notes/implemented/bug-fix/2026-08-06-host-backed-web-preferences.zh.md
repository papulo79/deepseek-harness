...
外观、语言和忙碌时 Enter 偏好跟随活跃 profile，跨刷新、端口和已认证 origin 保持一致。ConfigEditor 和 volatile HMR 应用 profile 更改；表单镜像通过 settings 失效通知跟随已接受值。旧浏览器偏好键 `dsh.theme`、`dsh.locale` 和 `dsh.conversation.busyEnter` 保持不使用。
