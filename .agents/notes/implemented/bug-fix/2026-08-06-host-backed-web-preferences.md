...
The Client disables Host persistence only for a composition that resolves memory mode; every authenticated browser — loopback or LAN-paired — uses it, so page locality does not change the transport ([later decision](../architecture/2026-09-15-host-settings-for-authenticated-browsers.md)). Dynamic third-party theme ids remain in-process extensions outside the built-in Host schema; removing one resets the live registry without replacing the last durable built-in preference.
...
Appearance, Language, and busy-Enter choices follow the active profile across reloads, ports, and authenticated origins. ConfigEditor and volatile HMR apply profile changes; the form mirror follows accepted values through settings invalidations. Legacy `dsh.theme`, `dsh.locale`, and `dsh.conversation.busyEnter` browser preference keys remain unused.
